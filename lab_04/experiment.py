...
class Experiment():
    ...
    def params_convert(self, gen_int, gen_var, pm_int, pm_var):
        m1, s1 = self.convert_to_normal_param(gen_int[0], gen_var[0])
        m2, s2 = self.convert_to_normal_param(gen_int[1], gen_var[1])

        a1, b1 = self.convert_to_unif_param(pm_int[0], pm_var[0])
        a2, b2 = self.convert_to_unif_param(pm_int[1], pm_var[1])

        return [m1, m2], [s1, s2], [a1, a2], [b1, b2]
