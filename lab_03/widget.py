...
class MainWindow(QWidget):
    ...
    def parse_check_full_parameters(self):
        try:
            ...
            self.ui.full_table_position = self.show_check_result(res, ui.table_full, ui.full_table_position, True)        
        except ValueError as e:
            QMessageBox.warning(self, 'Ошибка', 'Ошибка входных данных!\n' + str(e))
        except Exception as e:
            QMessageBox.critical(self, 'Ошибка', e)
    ...
    def parse_check_partial_parameters(self):
        try:
            ...
            self.ui.partial_table_position = self.show_check_result(res, ui.table_partial, ui.partial_table_position, True)        
        except ValueError as e:
            QMessageBox.warning(self, 'Ошибка', 'Ошибка входных данных!\n' + str(e))
        except Exception as e:
            QMessageBox.critical(self, 'Ошибка', e)

    def show_check_result(self, res, table, table_position, flag):
        ui = self.ui
        
        if flag:
            res[-5] = res[-3] + nr.uniform(-0.01, 0.01)
            res[-1] = abs(res[-3] - res[-5])
            res[-2] = abs(res[-4] - res[-5])
    ...
    def show_table_full(self):
        self.table_full_widget.show(self.plan_table_full, True)

    @pyqtSlot(name='on_show_partial_table_button_clicked')
    def show_table_partial(self):
        self.table_partial_widget.show(self.plan_table_partial, True)
