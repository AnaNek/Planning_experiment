...
class Modeller:
    def __init__(self, m, sigma, a, b,):
        self._generator = Generator(GaussDistribution(m, sigma))
        self._processor = Processor(UniformDistribution(a, b))
...
class Experiment():
    ...
    def param_convert(self, generator_intensity, generator_range, processor_intensity, processor_range):
        m = 1 / generator_intensity
        sigma = (1 / (generator_intensity - generator_range) - 1 / (generator_intensity + generator_range)) / 2
    ...
    def calculate(self):
        ...
        for exp in matrix:
            ...
            m, sigma, a, b = self.param_convert(gen_int, gen_var, pm_int, pm_var)

            exp_res = 0
            for i in range(MOD_NUMBER):
                model = Modeller(m, sigma, a, b)
    ...
    def check(self, gen_int, gen_var, pm_int, pm_var):
        ...
        for i in range(MOD_NUMBER):
            ...
            m, sigma, a, b = self.param_convert(new_gen_int, new_gen_var, new_proc_int, new_proc_var)
            model = Modeller(m, sigma, a, b)
            ro, avg_wait_time = model.event_based_modelling(self.time)
            exp_res += avg_wait_time
        ...
        return [gen_int, gen_var, pm_int, pm_var, exp_res, lin_res, nonlin_res]     # lin_res и nonlin_res - результаты решения уравнений
...
class MainWindow(QWidget):
    ...
    def _parse_parameters(self):
        try:
            ...
            self._show_results(table, regr)  # regr - коэффициенты уравнения
